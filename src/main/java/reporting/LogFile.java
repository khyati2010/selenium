...
public class LogFile {
	...
	public void prepareLogger(String FilePath) {
		try {
			...
			FileAppender appender = new FileAppender(new PatternLayout(),
					myLogFile.getAbsolutePath(), true);
	...
	public void prepareHeader() {
		log.info("TestCaseID" + "\t\t\t\t\t\t\t" + "TestStep"
				+ "\t\t\t\t\t\t\t" + "BDD_Step" + "\t\t\t\t\t\t\t" + "Status"
				+ "\t\t\t\t\t\t\t" + "Remarks" + "\n");
	}

	public void writeStepLog(String TestCaseID, String TestStep, String Status,
			String Remarks, String BDDStep, boolean flag) {
		if (flag) {
			log.info(TestCaseID + "\t\t\t\t\t\t\t" + TestStep
					+ "\t\t\t\t\t\t\t" + BDDStep + "\t\t\t\t\t\t\t" + Status
					+ "\t\t\t\t\t\t\t" + Remarks + "\n");
